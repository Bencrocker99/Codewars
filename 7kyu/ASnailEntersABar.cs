...
public static class Kata
{
    ...
    {
        const int MINUTES_IN_A_YEAR = 60 * 24 * 365;

        double totalTravelledDistance = 0;
        double rubberBandLength = x;
        int numberOfMinutes = 0;

        while (totalTravelledDistance < rubberBandLength)
        {
            numberOfMinutes++;

            totalTravelledDistance += y;
            rubberBandLength += z;

            if (totalTravelledDistance >= rubberBandLength)
                break;
            else if (numberOfMinutes >= MINUTES_IN_A_YEAR)
                return false;
        }
